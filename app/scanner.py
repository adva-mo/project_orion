...
from app.data import fetch_ticker
from app.fibonacci import detect_swing_low_confluence, get_best_fib
from app.indicators import compute_atr, compute_ema, compute_volume_profile, compute_vwap, filter_hvn
...
class ScanDetail:
    ...
    vp_from_date: str   # start of hourly window used for volume profile
    vp_to_date: str     # end of hourly window (most recent bar)
...
def scan_ticker_detail(ticker: str, min_rr: float = 2.0) -> ScanDetail:
    ...
    return ScanDetail(result=result, atr=atr, vwap=vwap, ema_20=ema_20, volume_profile=vp,
                      vp_from_date=vp_from_date, vp_to_date=vp_to_date)
...
def scan_tickers(
    ...
    for ticker in tickers:
        try:
            detail = scan_ticker_detail(ticker, min_rr)
